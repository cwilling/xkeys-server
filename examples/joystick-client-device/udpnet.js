...
var UdpNet = GObject.registerClass ({
	...
	_init(address, port, ui) {
		...
		this.discovered_hosts = [];
		this.ui = ui;
		/*	Details of device */
		this.vendor_id = 1523;
		this.product_id = 1065;
		this.unit_id = 128;
		this.device_name = "Remote XK-12 Joystick";
		this.row_count = 3;
		this.column_count = 4;
		...
		this.socket_source.set_callback( () => {
			try {
				...
				if ('TextDecoder' in window) {	// Older cjs versions don't have Text{En,De}coder
				...
				switch (msg.msg_type) {
					case "discover_result":
						/*
						*	Add responding server to discovered_hosts[]
						*	and choose which one to connect to later.
						*/
						log(`Received a discover_result message. Server address: ${msg.xk_server_address}`);
						msg["rinfo"] = rinfo;
						this.discovered_hosts.push(msg);
						break;
					case "connect_result":
						//this.connect_server_confirmed(msg, rinfo);
						//break;
					case "device_connect_result":
						this.device_connect_server_confirmed(msg, rinfo);
						break;
	...
	send_discover_message () {
		...
		try {
			...
			if ('TextEncoder' in window) {	// Older cjs versions don't have Text{En,De}coder
				const encoder = new TextEncoder();
				this.socket.send_to(Gio.InetSocketAddress.new(Gio.InetAddress.new_from_string('255.255.255.255'), this.service_port), encoder.encode(discover_message), null);
			} else {
				const ByteArray = imports.byteArray;
			...
			log(`XXX send_discover_message XXX: ${err}`);
		}
	}
	...
	/*	Send message to all servers we're connected to
	*/
	send_client_udp_message (message) {
		this.server_connections.forEach( (rinfo) => {
			this.send_udp_message(message, rinfo);
		});
	}
	...
	*/
	discover_result_connect (msg, rinfo, myname) {
		...
		this.send_client_udp_message(connect_message, rinfo);
	}

	/*	choose_server (server_id)
	*	
	*	A device client should only connect to a single server.
	*	Here we choose the server to connect to. If a server name
	*	has been supplied, respond only to that server.
	*	Otherwise (no preference supplied) connect to the
	*	first server to respond to the discovery message.
	*/
	choose_server (server_id) {
		log(`choose_server() target = ${server_id}`);
		log(`choose_server() discovered_hosts = ${JSON.stringify(this.discovered_hosts)}`);
		if (this.discovered_hosts.length == 0) {
			/*	No servers found so, after 1 second, try again */
			GLib.timeout_add_seconds(GLib.PRIORITY_DEFAULT, 1, () => { this.choose_server(server_id); return GLib.SOURCE_REMOVE; });
			log(`No servers found - trying again soon ...`);

			/*	Perhaps, after some number of failures, we should send another discovery message? */

		} else {
			log(`Found some`);
			if (server_id) {
				/*	Extract the entry with matching SID */
				const target = this.discovered_hosts.find(entry => { return entry.server_id === server_id ; });
				log(`target = ${JSON.stringify(target)}`);
				var device_connect_message = JSON.stringify({"msg_type":"device_connect", "device":this.device_name, "vendor_id":this.vendor_id, "product_id":this.product_id,"unit_id":this.unit_id, "rowCount":this.row_count, "colCount":this.column_count});
				this.send_udp_message(device_connect_message, target.rinfo);
			} else {
			}
		}
	}

	/*	connect_server_confirmed()
	*
	...
	*/
	...
	/*	device_connect_server_confirmed()
	*
	*	Similar to connect_server_confirmed()
	*	but as a "device", don't really need to know about other devices.
	*/
	device_connect_server_confirmed (msg, rinfo) {
		log(`connect_server_confirmed(): ${JSON.stringify(msg)}`);
		const index = this.server_connections.findIndex(item => item.address === rinfo.address && item.port === rinfo.port);
		if (index < 0) {	// Not yet connected to this server
			log(`Now connected to ${msg.server_id} at ${rinfo.address}:${rinfo.port}`);
